...
\end{itemize}
Die Verteilungsfunktion der Exponentialverteilung \cite{Behrends2012}
$$F(X)=1-e^{-\lambda x}$$
berechnet die Auftrittswahrscheinlichkeit des nächsten Ereignisses im Intervall $[0;x]$. Die Dichtefunktion ist gegeben durch \cite{Behrends2012} 
$$f(x)= 
...
Die Inverse ist definiert durch \cite{Behrends2012} 
...
\end{itemize}
Die Verteilungsfunktion der Normalverteilung ist gegeben durch \cite{Inversionsmethode} 
$$\frac{1}{\sigma\sqrt{2\pi}} \int_{-\infty}^x e^{-\frac{1}{2} \left(\frac{t-\mu}{\sigma}\right)^2}$$
Die Normalverteilung wird mit der Dichtefunktion \cite{Inversionsmethode} 
$$f(x,\mu,\sigma^2)=\frac{1}{\sigma\sqrt{2\pi}}e^{-\frac{1}{2}(\frac{x-\mu}{\sigma})^2}$$
dargestellt. Die folgende Abbildung \ref{fig:normalverteilung} stellt die Dichtefunktion der Normalverteilung $\mathcal{N}(0,1)$ dar.

\begin{figure}[H]
    \centering
    \begin{tikzpicture} 
        \begin{axis}[domain=-5:5,samples=100] 
          \addplot[black] {1/(1*sqrt(2*pi))*exp(-.5*((x-0)/1)^2)} ; 
        \end{axis} 
    \end{tikzpicture} 
...
\begin{figure}[H]
    \centering
    \includegraphics[height=.55\textwidth]{spline_pdf.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=.55\textwidth]{spline_cdf.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=.55\textwidth]{spline_cdf_norm.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=.55\textwidth]{spline_cdf_norm_ex.png}
...
\subsubsection{Zufallszahlengenerierung für diskrete Wahrscheinlichkeitsverteilungen}
...
Wählt man zwei aufeinanderfolgende Kontrollpunkte der diskreten Wahrscheinlichkeitsverteilung $(x_n,y_n)$ und $(x_{n+1},y_{n+1})$, dann ist der Flächeninhalt $A_n$ definiert durch $A_n=(x_{n+1}-x_n) \cdot y_n$.
Da bei einer diskreten Wahrscheinlichkeitsverteilung der Abstand zwischen benachbarten diskreten Werten gleich $1$ ist, genügt es, die einzelnen y-Werte kumulativ aufzuaddieren, um die Verteilungsfunktion zu bestimmen.
